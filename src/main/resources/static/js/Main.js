...
window.onload = function() {
  ...
  function addWorldPlane() {
    ...
    mtlLoader.load("textures/tiles/" + tiles[Math.floor(Math.random() * tiles.length)] + ".mtl", function(materials) {
      ...
      objLoader.load("textures/tiles/" + tiles[Math.floor(Math.random() * tiles.length)] + ".obj", function(mesh) {
        ...
        while (randomnumberx < 20 && randomnumberx > -20) {
        	randomnumberx = Math.random() * 300;
        	if (Math.random() > 0.5)
        		randomnumberx *= -1;
        }
        while (randomnumberz < 20 && randomnumberz > -20) {
        	randomnumberz = Math.random() * 300;
        	if (Math.random() > 0.5)
        		randomnumberz *= -1;
        }
        scene.add(mesh);
  ...
  function addWarehouse() {
  	  //logo
    mtlLoader.load("textures/logo/Capture.mtl", function(materials) {
      materials.preload();
      const objLoader = new THREE.OBJLoader();
      objLoader.setMaterials(materials);
      objLoader.load("textures/logo/Capture.obj", function(mesh) {
        mesh.traverse(function(node) {
          if (node instanceof THREE.Mesh) {
            node.castShadow = true;
            node.receiveShadow = true;
